                ...
                <div class="box-body">
                  <table class="table table-striped">
                    ...
                    <tbody>
                      ...
                      <tr
                        v-else
                        v-for="(purchas, index) in purchase_items.data"
                        :key="index"
                      >
                        <td>{{ index + 1 }}</td>
                        <td>
                        ...
                        <td>
                          <a :href="'/backend/sale/view/' + purchas.id" target="_blank">
                            {{ "S-" + purchas.invoice_no }}
                          </a>
                        </td>
                        <td>
                          {{ ProductCount(purchas.purchase_items) }}
                        </td>
                        <td>{{ purchas.created_at }}</td>
                        <td>
                          {{ parseInt(purchas.total) }}
                        </td>
                      </tr>
                      <tr>
                        <td colspan="3"></td>
                ...
                <div class="box-footer">
                  <div class="row">
                    ...
                    >
                      <p>
                        Showing
                        <strong>{{ purchase_items.from }}</strong> to
                        <strong>{{ purchase_items.to }}</strong> of total
                        <strong>{{ purchase_items.total }}</strong> entries
                      </p>
                    </div>
                  </div>
                </div>
...
export default {
  ...
  methods: {
    ...
    totalAmount() {
      let total = 0;
      this.purchase_items.data.forEach((element) => {
    ...
    totalproductQuantity() {
      let total = 0;
      this.purchase_items.data.forEach((element) => {
        element.purchase_items.forEach((ele) => {
